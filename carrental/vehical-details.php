...
// Capture kilometers and driver requirement
$kmToTravel = $_POST['kmToTravel']; // kilometers user intends to travel
$requiresDriver = isset($_POST['requiresDriver']) ? 1 : 0; // 1 if user requires a driver, otherwise 0
...
if($query1->rowCount()==0)
{
  ...
  $sql = "INSERT INTO tblbooking(BookingNumber, userEmail, VehicleId, FromDate, ToDate, message, KmToTravel, RequiresDriver, Status) 
  VALUES(:bookingno, :useremail, :vhid, :fromdate, :todate, :message, :kmToTravel, :requiresDriver, :status)";
...
$query->bindParam(':kmToTravel', $kmToTravel, PDO::PARAM_INT);
$query->bindParam(':requiresDriver', $requiresDriver, PDO::PARAM_INT);
    ...
    <div class="booking-form">
        ...
        <form method="post" name="booking">
            <div class="form-group">
                ...
                <div>
                    <input type="radio" name="rentalType" value="days" id="rentalDays" checked onclick="toggleBookingOptions()">
                    <label for="rentalDays">Day-wise</label>
                    <input type="radio" name="rentalType" value="km" id="rentalKm" onclick="toggleBookingOptions()">
                    <label for="rentalKm">Kilometers</label>
                </div>
            </div>

            <!-- Day-wise booking options -->
            <div id="dayBooking" class="booking-options">
                <div class="form-group">
            ...
            <!-- Km-wise booking options -->
            <div id="kmBooking" class="booking-options" style="display:none;">
                <div class="form-group">
                    <label for="kmToTravel">Km to Travel</label>
                    <input type="number" class="form-control" name="kmToTravel" required>
                </div>
            </div>
            ...
            <div class="form-group">
                <label for="requiresDriver">Requires Driver</label>
                <input type="checkbox" name="requiresDriver" value="1"> Yes
            </div>
...
function toggleBookingOptions() {
    ...
    const kmBooking = document.getElementById('kmBooking');

    if (rentalType === 'days') {
        dayBooking.style.display = 'block';
        kmBooking.style.display = 'none';
    } else {
        dayBooking.style.display = 'none';
        kmBooking.style.display = 'block';
    }
}
</script>
